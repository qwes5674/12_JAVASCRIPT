...
/* 
정규 표현식(Regular expression)은 일정한 패턴을 가진 문자열의 
집합을 표현하기 위해 사용하는 형식 언어(formal language)이다.
...
//  정규표현식
// 정규표현식 리터럴 :  /패턴/플래그 : 
let regexp = /j/i; // 패턴  : j , i -> 플래그 (속성), 대소문자 구분없이  /.../ -> 검색할 정규표현식(패턴)
...
// test 메서드 - target 문자열에 대해 정규 표현식 regexp의 패턴을 검색하여 매칭결과를 Boolean으로 반환
