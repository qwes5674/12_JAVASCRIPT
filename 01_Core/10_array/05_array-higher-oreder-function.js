...
// 배열이 기본적으로 문자열 정렬리 되므로, 한자리수, 세자리수가 올바르게 정렬되지 않음
...
제거하여 복잡성을 해결하고 변수의 사용을 억제하여 상태 변경을 피하려는 프로그래밍 패러타임
*/
...
// JavaScript에서는 조건문에 % 2 -> 짝수면 0 홀수면 1
...
// Array.prototype.every : 배열의 모든 요소가 콜백함수의 테스트를 통과하는지 확인 (Boolean)
...
// 배열을 순환혐 각 요소에 대한 인자로 주어진 콜백함수를 실행 그 결과가 참인 첫번째 요소를 반환
// 만약, 참인 요소가 존재하지 안으면 undefined

//Array.prototype.findIndex :
// 배열을 순환혐 각 요소에 대한 인자로 주어진 콜백함수를 실행 그 결과가 참인 첫번째 요소의 인덱스를 반환
// 만약, 참인 요소가 존재하지 안으면 -1
...
// 60점 이상의 학생들을 전부 알고싶으면 ?
// filter는 콜백함수의 실행결과가 true인 배열 요소 값만 추출해서 배열로 반환
